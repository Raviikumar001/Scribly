...
const EditorComponent: React.FC<Props> = ({ setSidebar, setCounter, activeNote, notes, setActiveNote }) => {
  ...
  return (
    <div className="h-[100%] w-full">
      <div className={activeNote ? "p-3 flex justify-between border-b-2 border-gray-150 md:border-2  " : "hidden"}>
        <>
          <div className="md:hidden">
            <Link to="/app">
              <span onClick={headBackToApp}>
                {" "}
                <BackArrow />{" "}
              </span>
            </Link>{" "}
          </div>
          <div className="hidden md:block" onClick={() => setSidebar((prev) => !prev)}>
            {" "}
            <Toggle />
          </div>
      ...
      {/* Action component */}
      <div>{showchecked && <ActionComponent setCounter={setCounter} ischecked={ischecked} setShowChecked={setShowChecked} updateCheck={toggleCheckbox} activeNote={activeNote} setActiveNote={setActiveNote} />}</div>

      <div className={ischecked ? " flex justify-center items-center h-screen md:border md:border-slate-300" : "hidden"}>
        <div className="text-gray-700 text-lg w-[80%] ml-9 pl-5 pr-3 h-[87%] mt-4 border border-transparent  focus:outline-none">
      ...
      <div className={ischecked ? "hidden" : "block"}>
        {activeNote ? (
          <form className="h-screen md:border md:border-slate-300">
            <input type="text" placeholder="Enter Title" value={title} onChange={editTitle} className="text-slate-700 pl-5 pr-3 pt-8 w-[80%] placeholder:text-lg border border-transparent focus:outline-none" />
            <textarea name="notes" value={body} onChange={editBody} className="text-gray-700 text-lg w-full  pl-5 pr-3 h-[87%] mt-4 border border-transparent  focus:outline-none" placeholder="Note..."></textarea>
          </form>
        ) : (
          <div className="flex justify-center items-center h-screen md:border md:border-slate-300 text-slate-600 text-lg">Select or create a Note</div>
        )}
      </div>
    </div>
  );
